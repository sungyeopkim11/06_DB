...
-- SELECT도 DML에 포함되긴 하지만 DQL이라는 것으로 분리해서 보기도 한다!
   --> DQL(Data Qurey Language : 데이토 질의 언어)
...
ROLLBACK;
--> COMMIT 되지 않는 DML(INSERT,UPDATE,DELETE) 취소하기
 -- == DML 수행 전으로 되돌리기
...
-- 인인벨 ( INSERT  INTO 테이블명  VALUES(모든 컬럼에 추가할 값) )
-- 인인벨 ( INSERT  INTO 테이블명(컬럼명)  VALUES(INTO에 작성한 컬럼에 추가할 값) )
...
-- 업셋웨 UPDATE 테이블명 SET 컬럼명 = 수정할 값 WHERE = 컬럼명 비교 연산자	
...
-- 델프웨 DELETE FROM 테이블명 WHERE 삭제하려는 행을 찾는 조건
