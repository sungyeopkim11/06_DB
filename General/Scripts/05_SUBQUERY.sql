...
SELECT 이름, 급여
FROM (SELECT EMP_NAME 이름, SALARY 금여
      FROM EMPLOYEE)
WHERE 급여 >= 4000000;         
...
-- 보통의 SELECT문은 서브쿼리 -> 메인쿼라 순서로 해석하는데 
-- 상관 서브쿼리는
...
FROM EMPLOYEE MAIN -- 메인쿼리 별칭이 MAIN
WHERE SALARY > (
	SELECT AVG(SALARY)
	FROM EMPLOYEE SUB -- 서브쿼리 별칭이 SUB
...
-- 1) RANK() OVER
-- 			OVER() 괄호에 작석되 정렬 기준대로 정렬 후 순위 지정
