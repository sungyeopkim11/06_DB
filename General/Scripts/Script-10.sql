...
SELECT 
		...
		(SELECT IMG_PATH || IMG_RENAME
		 FROM BOARD_IMG I
		 WHERE I.BOARD_NO = 2017 ) AS THUMBNAIL
...
-- 형제들(같은 레벨) 간의 정렬 순서를 COMMENT_NO 오름차순으로 지정
...
CREATE TABLE "NOTIFICATION" (
	...
	"RECEIVE_MEMBER_NO2"	NUMBER		NOT NULL
);

COMMENT ON COLUMN "NOTIFICATION"."NOTIFICATION_NO" IS '알림 번호';

COMMENT ON COLUMN "NOTIFICATION"."NOTIFICATION_CONTENT" IS '알림 내용(HTML포함)';
...
COMMENT ON COLUMN "NOTIFICATION"."RECEIVE_MEMBER_NO2" IS '알림 받는 회원 번호';
...
SELECT COUNT(*)
		...
		AND   RECEIVE_MEMBER_NO = 6;


	-- 회원 프로칠 이미지 + 게시글 이미지 파일명만 모두 조회
...
CREATE TABLE "CHATTING_ROOM" (
	...
	"CHATTING_DATE"	DATE	DEFAULT CURRENT_DATE	NOT NULL,
	"OPEN_MEMBER"	NUMBER		NOT NULL,
	"PRATICIPANT"	NUMBER		NOT NULL
);

COMMENT ON COLUMN "CHATTING_ROOM"."CHATTING_ROOM_NO" IS '채팅방 번호';

COMMENT ON COLUMN "CHATTING_ROOM"."CHATTING_DATE" IS '채팅방 생성일';

COMMENT ON COLUMN "CHATTING_ROOM"."OPEN_MEMBER" IS '개설자 회원 번호';

COMMENT ON COLUMN "CHATTING_ROOM"."PRATICIPANT" IS '참여자 회원 번호';
