...
/* 자료형
    ...
    ** CHAR/VARCHAR2 주의사항 **
    - 오라클은 기본적으로 UTF-8 문자 인코딩을 사용하는데
      UTF-8은 영어/숫자 1BYTE, 한글 3BYTE로 저장하기 떄문에
      한글 데이터 저장 시 크기 계산을 주의해야 한다!!
      
    NVARVHAR2(문자 개수) : "가변" 길이 문자형 (4000BYTE)
     -  BYTE크기 단위가 아닌 글자 수(문자개수)로 크기를 지정하는 타입
     
    EX) VARCHAR2(3)  : 가변길이 문자형 크기 3BYTE    
        NVARCHAR2(3) : 가변길이 문자형 3개까지 저장 가능
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL, -- 컬럼 레벨 제약 조건
...
-- ORA-00001: 무결성 제약 조건(KH_KSY.USER_ID_NAME_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_KSY.PK_USERNO_USERID)에 위배됩니다
...
'010-6666-1234', 'yoon123@kh.
or.kr', 50);
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 -- 외래키 제약 조건에 위배되어 오류 발생.
-- ORA-02291: 무결성 제약조건(KH_KSY.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
SELECT * FROM USER_USED_FK2; -- 10이 NULL로 변환
...
VALUES (1, '1Q2W3E', 'ASDQWE', '990616-1167511', '남', '010-9637-3631', NULL, 'N');
INSERT INTO USER_TEST
 ...
 * NUMBER : 정수/실수
 * 
 * CHAR(크기)     : 고정 길이 문자열 최대 2000BYTE
 * VARCHAR2(크기) : 가변 길이 문자열 최대 4000BYTE
 * 
